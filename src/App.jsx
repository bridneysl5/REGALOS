...
const App = () => {
  ...
  const [activeFilter, setActiveFilter] = useState({
    ...
    occasion: initialRoute.occasion,
  ...
  const [selectedProduct, setSelectedProduct] = useState(() =>
    findProductBySlug(ALL_PRODUCTS, initialRoute.productSlug)
  );
  const [pendingProductSlug, setPendingProductSlug] = useState(() =>
    initialRoute.productSlug && !findProductBySlug(ALL_PRODUCTS, initialRoute.productSlug)
      ? initialRoute.productSlug
      : null
  );
  ...
  // El producto puede vivir en Firebase: lo resolvemos cuando llega el catálogo.
  useEffect(() => {
    if (!pendingProductSlug) return;
    const found = findProductBySlug(combinedProducts, pendingProductSlug);
    if (found) {
      setSelectedProduct(found);
      setPendingProductSlug(null);
    } else if (firebaseLoaded) {
      setPendingProductSlug(null); // ya no existe: mostramos el catálogo
    }
  }, [combinedProducts, pendingProductSlug, firebaseLoaded]);
  ...
  useEffect(() => {
    ...
    if (pendingProductSlug && !selectedProduct) return; // esperamos al producto

    const target = buildUrl({
      view,
      category: activeFilter.category,
      occasion: activeFilter.occasion,
    ...
    const onlyQueryChanged =
      previous && previous.split('?')[0] === target.split('?')[0];

    if (previous === null || onlyQueryChanged) {
      // Primera carga (incluye enlaces antiguos) o sólo cambió la búsqueda.
  ...
  useEffect(() => {
    const onPop = () => {
      ...
      const product = findProductBySlug(combinedProducts, route.productSlug);
      ...
      setPendingProductSlug(route.productSlug && !product ? route.productSlug : null);
    };
  ...
  const occasions = ['Todos', ...OCCASIONS];
